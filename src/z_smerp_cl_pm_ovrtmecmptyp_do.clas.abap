class Z_SMERP_CL_PM_OVRTMECMPTYP_DO definition
  public
  inheriting from /SYCLO/CL_CORE_DT_HANDLER
  create public .

public section.
*"* public components of class Z_SMERP_CL_PM_OVRTMECMPTYP_DO
*"* do not include other source files here!!!
  ...
  methods /SYCLO/IF_CORE_FILTER_SERV~GET_DATA_FILTER_LIST
    redefinition .
protected section.
*"* protected components of class Z_SMERP_CL_PM_OVRTMECMPTYP_DO
*"* do not include other source files here!!!

  methods GET_DATA_TABLE
    redefinition .
*"* private components of class Z_SMERP_CL_PM_OVRTMECMPTYP_DO
*"* do not include other source files here!!!
private section.
ENDCLASS.
...
CLASS Z_SMERP_CL_PM_OVRTMECMPTYP_DO IMPLEMENTATION.
...
*Field Filter for method GET_DATA_TABLE
  ...
  <data_filter>-dof_name = 'VME'.
  <data_filter>-usage_tabname = 'T555T'.
  <data_filter>-usage_fieldname = 'VME'.
...
*Tables & Structures
  ...
  DATA: BEGIN OF ls_dof_filter_vals,
          vme     TYPE REF TO /syclo/core_range_tab,
...
*********************************************************************
  TRY.
      ...
      "EAM #58326 GYMANA - modify select to use T555R config table and
      "T555T to get the text description.

      SELECT t555r~vme AS key
             t555t~txt AS value
        FROM t555r INNER JOIN t555t
          ON t555r~vme = t555t~vme
        INTO CORRESPONDING FIELDS OF TABLE lt_data_table
        WHERE t555t~spras = sy-langu AND
              t555r~vme   IN ls_dof_filter_vals-vme->*.
