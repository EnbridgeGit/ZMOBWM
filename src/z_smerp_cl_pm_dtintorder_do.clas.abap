class Z_SMERP_CL_PM_DTINTORDER_DO definition
  public
  inheriting from /SYCLO/CL_CORE_DT_HANDLER
  create public .

public section.
*"* public components of class Z_SMERP_CL_PM_DTINTORDER_DO
*"* do not include other source files here!!!
  ...
  methods /SYCLO/IF_CORE_FILTER_SERV~GET_DATA_FILTER_LIST
    redefinition .
protected section.
*"* protected components of class Z_SMERP_CL_PM_DTINTORDER_DO
*"* do not include other source files here!!!

  methods GET_DATA_TABLE
    redefinition .
*"* private components of class Z_SMERP_CL_PM_DTINTORDER_DO
*"* do not include other source files here!!!
private section.
ENDCLASS.
...
CLASS Z_SMERP_CL_PM_DTINTORDER_DO IMPLEMENTATION.
...
*Field Filter for method GET_DATA_TABLE
  ...
  <data_filter>-dof_name = 'COMPANY_CODE'.
  <data_filter>-usage_tabname = 'AUFK'.
  <data_filter>-usage_fieldname = 'BUKRS'.

*Field Filter for method GET_DATA_TABLE
  APPEND INITIAL LINE TO lt_data_filter ASSIGNING <data_filter>.
  <data_filter>-do_handler = me->clsname.
  <data_filter>-do_mthd = 'GET'.
  <data_filter>-dof_name = 'ORDER_TYPE'.
  <data_filter>-usage_tabname = 'AUFK'.
  <data_filter>-usage_fieldname = 'AUART'.
...
*Tables & Structures
  ...
  DATA: BEGIN OF ls_dof_filter_vals,
          company_code     TYPE REF TO /syclo/core_range_tab,
          order_type       TYPE REF TO /syclo/core_range_tab,
...
*********************************************************************
  TRY.
      ...
      SELECT aufnr AS key
        ...
        WHERE auart IN ls_dof_filter_vals-order_type->*
          AND bukrs IN ls_dof_filter_vals-company_code->*
          AND autyp = '01'
          AND ( phas2 = 'X' OR phas1 = 'X' ).  "Changes done for Defect 220-Nidhideep
      ...
      loop at lt_data_table into ls_data_table.
        lv_aufnr = ls_data_table-key.
        CALL FUNCTION 'CONVERSION_EXIT_AUFNR_OUTPUT'
          EXPORTING
            input         = lv_aufnr
          IMPORTING
            OUTPUT        = ls_data_table-key.

        MODIFY lt_data_table from ls_data_table.
      ENDLOOP.
