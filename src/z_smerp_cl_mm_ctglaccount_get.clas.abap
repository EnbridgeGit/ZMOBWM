class Z_SMERP_CL_MM_CTGLACCOUNT_GET definition
  public
  inheriting from /SYCLO/CL_CORE_CT_HANDLER
  create public .

public section.
*"* public components of class Z_SMERP_CL_MM_CTGLACCOUNT_GET
*"* do not include other source files here!!!
  ...
  methods /SYCLO/IF_CORE_FLDSEL_SERV~GET_FIELD_SELECTOR_TABLES
    redefinition .
protected section.
*"* protected components of class Z_SMERP_CL_MM_CTGLACCOUNT_GET
*"* do not include other source files here!!!

  methods GET_COMPLEX_TABLE
    redefinition .
private section.
*"* private components of class Z_SMERP_CL_MM_CTGLACCOUNT_GET
*"* do not include other source files here!!!
ENDCLASS.



CLASS Z_SMERP_CL_MM_CTGLACCOUNT_GET IMPLEMENTATION.
...
*Field selectors
  ...
  <data_filter>-dof_name        = 'KOKRS'.
  <data_filter>-usage_tabname   = 'CSKB'.
  <data_filter>-usage_fieldname = 'KOKRS'.

  APPEND INITIAL LINE TO lt_data_filter ASSIGNING <data_filter>.
  <data_filter>-do_handler      = me->clsname.
  <data_filter>-do_mthd         = 'GET'.
  <data_filter>-dof_name        = 'KSTAR'.
  <data_filter>-usage_tabname   = 'CSKB'.
  <data_filter>-usage_fieldname = 'KSTAR'.
...
*OO Reference Variables
  ...
  DATA: lt_cskb TYPE STANDARD TABLE OF zsmerp_mm_glaccount_str.
*Variables
  DATA: lv_select_clause TYPE string,
        ls_return        TYPE bapiret2.

  DATA: BEGIN OF ls_mdo_output_vals,
        et_complex_table              TYPE REF TO zsmerp_mm_glaccount_tab,
      END OF ls_mdo_output_vals.

  DATA: BEGIN OF ls_dof_filter_vals,
      kokrs TYPE REF TO /syclo/core_range_tab,
      kstar TYPE REF TO /syclo/core_range_tab,
...
* Get the key fields
      ...
      SELECT b~kokrs b~kstar u~ltext AS ktext
        INTO CORRESPONDING FIELDS OF TABLE lt_cskb
        FROM cskb AS b
          INNER JOIN csku AS u ON u~kstar = b~kstar
       WHERE b~kokrs IN ls_dof_filter_vals-kokrs->*
         AND b~kstar IN ls_dof_filter_vals-kstar->*
         AND u~spras = sy-langu
         AND ( b~datab <= sy-datum AND b~datbi >= sy-datum )
         AND deakt = space
         AND loevm = space
         AND katyp = '01'.

      IF lt_cskb[] IS INITIAL.
      ...
      APPEND LINES OF lt_cskb TO ls_mdo_output_vals-et_complex_table->*.
