class Z_SMERP_CL_MM_CTAGREEMENT_GET definition
  public
  inheriting from /SYCLO/CL_CORE_CT_HANDLER
  create public .

public section.
*"* public components of class Z_SMERP_CL_MM_CTAGREEMENT_GET
*"* do not include other source files here!!!
  ...
  methods /SYCLO/IF_CORE_FLDSEL_SERV~GET_FIELD_SELECTOR_TABLES
    redefinition .
protected section.
*"* protected components of class Z_SMERP_CL_MM_CTAGREEMENT_GET
*"* do not include other source files here!!!

  methods GET_COMPLEX_TABLE
    redefinition .
*"* private components of class Z_SMERP_CL_MM_CTAGREEMENT_GET
*"* do not include other source files here!!!
private section.
ENDCLASS.
...
CLASS Z_SMERP_CL_MM_CTAGREEMENT_GET IMPLEMENTATION.
...
*Field selectors
  ...
  <data_filter>-usage_tabname   = 'ZCONTRACTS'.
  <data_filter>-usage_fieldname = 'MATKL'.

  APPEND INITIAL LINE TO lt_data_filter ASSIGNING <data_filter>.
  <data_filter>-do_handler      = me->clsname.
  <data_filter>-do_mthd         = 'GET'.
  <data_filter>-dof_name        = 'PLANT'.
  <data_filter>-usage_tabname   = 'ZCONTRACTS'.
  <data_filter>-usage_fieldname = 'WERKS'.

  APPEND INITIAL LINE TO lt_data_filter ASSIGNING <data_filter>.
  <data_filter>-do_handler      = me->clsname.
  <data_filter>-do_mthd         = 'GET'.
  <data_filter>-dof_name        = 'PURCHASE_ORG'.
  <data_filter>-usage_tabname   = 'ZCONTRACTS'.
  <data_filter>-usage_fieldname = 'EKORG'.
...
*Tables & Structures
  DATA: lt_zcontracts TYPE zsmerp_mm_konnr_tab.
...
*Variables
  ...
  DATA: BEGIN OF ls_mdo_output_vals,
        et_complex_table              TYPE REF TO zsmerp_mm_konnr_tab,
  ...
  DATA: BEGIN OF ls_dof_filter_vals,
      ...
      purchase_org   TYPE REF TO /syclo/core_range_tab,
...
* Get the key fields
      ...
      lv_select_clause = me->build_field_selector_string( iv_mthd = lc_mthd
                                                          iv_tabname = 'ZCONTRACTS').
      SELECT (lv_select_clause)
        INTO CORRESPONDING FIELDS OF TABLE lt_zcontracts
        FROM zcontracts
       WHERE matkl IN ls_dof_filter_vals-material_group->* AND
             werks IN ls_dof_filter_vals-plant->* AND
             ekorg IN ls_dof_filter_vals-purchase_org->*.

      IF lt_zcontracts IS INITIAL.
      ...
      APPEND LINES OF lt_zcontracts[] TO ls_mdo_output_vals-et_complex_table->*.
